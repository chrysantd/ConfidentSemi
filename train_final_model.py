...
def get_arguments():
    ...
    parser.add_argument("--dataset", type=str, default='cifar10',choices=['svhn','cifar10'],
                        help="dataset type")
    parser.add_argument("--model", type=str, default='wide28',choices=['wide22','wide28'],
                        help="dataset type")     
...
def train_with_mix2(trainloader,model,optimizer,epoch,logger=None):
    model.train()
    #global_step = epoch * len_iter
...
def main():
    ...
    optimizer2 = AdamW(model2.parameters(), lr=args.lr, weight_decay=args.weight_decay)

    model2_save_path = osp.join(args.snapshot_dir,'final_model.pth')        
    ...
    file_handler = logging.FileHandler(osp.join(args.snapshot_dir,'final_logs.txt'))
    ...
    train_dataset = torch.utils.data.ConcatDataset([unlabelset,labelset])

    batch_sampler = TwoStreamBatchSampler(
        np.arange(len(unlabelset)),
        np.arange(len(labelset)) + len(unlabelset),
        args.batch_size,
        args.batch_size//2
    )
    ...
    for epoch in range(args.max_epoch):
        ...
        if args.vat:
            ...
            train_with_mix2(trainloader,model2,optimizer2,epoch,logger)
